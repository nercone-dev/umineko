...
#[derive(Debug)]
pub struct UDSDatagramConnection {
    local: UDSPath,
    remote: UDSPath,
    limits: UDSLimits,
...
impl UDSDatagramConnection {
    pub const KIND: UDSType = UDSType::Datagram;

    pub async fn bind(local: UDSPath, limits: UDSLimits) -> Result<Self, UDSError> {
        Self::bind_with(local, &UDSServerConfig { kind: Self::KIND, ..UDSServerConfig::default() }, limits).await
    }
    ...
    pub async fn send_to(&mut self, remote: &UDSPath, data: &[u8]) -> Result<usize, UDSError> {
        match &self.backend {
            ProviderBackend::Builtin => todo!(),
    ...
    pub async fn receive_from(&mut self, data: &mut [u8]) -> Result<(usize, UDSPath), UDSError> {
        match &self.backend {
            ProviderBackend::Builtin => todo!(),
