...
#[derive(Debug)]
pub struct UDSSeqpacketConnection {
    local: UDSPath,
    remote: UDSPath,
    limits: UDSLimits,
...
impl UDSSeqpacketConnection {
    pub const KIND: UDSType = UDSType::Seqpacket;
...
#[derive(Debug)]
pub struct UDSSeqpacketListener {
    local: UDSPath,
    backlog: u32,
    limits: UDSLimits,
    backend: ProviderBackend<dyn UDSProvider>,
...
impl UDSSeqpacketListener {
    ...
    pub async fn accept(&mut self) -> Result<UDSSeqpacketConnection, UDSError> {
        match &self.backend {
            ProviderBackend::Builtin => todo!(),
            ProviderBackend::Handle { provider, handle } => {
                ...
                UDSSeqpacketConnection::from_provider(provider.clone(), accepted, self.limits)
            }
        }
    }
