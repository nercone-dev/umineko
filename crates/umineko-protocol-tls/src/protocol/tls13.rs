...
impl TLS13Connection {
    pub const VERSION: TLSVersion = TLSVersion::V1_3;
    ...
    pub async fn send_early(&mut self, data: &[u8]) -> Result<usize, TLSError> {
        match &self.backend {
            ProviderBackend::Builtin => todo!(),
            ProviderBackend::Handle { provider, handle } => Ok(provider.send_early(*handle, data)?),
        }
    }
    ...
    pub fn early_data_accepted(&self) -> bool {
        match &self.backend {
            ProviderBackend::Builtin => todo!(),
            ProviderBackend::Handle { provider, handle } => provider.early_data_accepted(*handle).unwrap_or(false),
        }
    }
}
